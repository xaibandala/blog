...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        // If a video URL was provided, append to content so the frontend auto-embeds it
        if ($videoUrl !== '') {
            $content = rtrim($content) . "\n\n" . $videoUrl;
        }
        $stmt = $pdo->prepare('INSERT INTO posts (title, content, author, category, tags, cover_image, created_at) VALUES (?, ?, ?, ?, ?, ?, NOW())');
        $stmt->execute([$title, $content, $author, $category !== '' ? $category : null, $tags !== '' ? $tags : null, $coverPath !== '' ? $coverPath : null]);
